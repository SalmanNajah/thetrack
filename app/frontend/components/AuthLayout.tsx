      ...
      <div className="flex flex-1 flex-col justify-center px-6 py-12 lg:w-1/2 lg:flex-none lg:px-20 xl:px-24 w-full relative z-10">
        <div className="mx-auto w-full max-w-md">
          <div className="mb-10 text-left">
            <Link
              href="/"
              className="inline-block text-2xl font-bold tracking-tight text-tt-text hover:opacity-80 transition-opacity lg:hidden"
            >
              TheTrack
            </Link>
            ...
            <p className="mt-2 text-sm text-tt-text-secondary">{subtitle}</p>
          </div>

          {flash?.notice && (
            <div className="mb-6  bg-emerald-50/50 px-4 py-3 text-sm text-emerald-800 ring-1 ring-emerald-200/50 backdrop-blur-xs">
              {flash.notice}
            </div>
          )}

          {flash?.alert && (
            <div className="mb-6  bg-red-50/50 px-4 py-3 text-sm text-red-800 ring-1 ring-red-200/50 backdrop-blur-xs">
              {flash.alert}
            </div>
          )}

          <div className=" border border-tt-border-subtle bg-white p-8 shadow-xs">
            {children}
          </div>
        </div>
      </div>
      ...
      <div className="relative hidden lg:block lg:w-1/2 bg-[#18181b] overflow-hidden select-none border-l border-white/5">
        ...
        <div className="relative h-full flex flex-col justify-between p-16 z-10 text-white">
          ...
          <div className="relative w-full max-w-sm mx-auto h-[320px]">
            <div className="absolute top-0 left-0 w-64  border border-white/5 bg-white/2 backdrop-blur-md px-5 py-4 shadow-xl -rotate-3 hover:rotate-0 hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="absolute top-24 right-0 w-64  border border-white/5 bg-white/2 backdrop-blur-md px-5 py-4 shadow-xl rotate-3 hover:rotate-0 hover:-translate-y-1 transition-all duration-300 z-20">
            ...
            <div className="absolute bottom-0 left-4 w-72  border border-white/5 bg-white/2 backdrop-blur-md p-5 shadow-xl -rotate-2 hover:rotate-0 hover:-translate-y-1 transition-all duration-300 z-10">
          ...
          <div className="max-w-md">
            ...
            <p className="mt-3 text-sm text-white/50 leading-relaxed font-sans">
              Organize your money into clear stashes, track transactions
              dynamically, and optimize your wealth with minimal overhead.
            </p>
          </div>
        </div>
      </div>
